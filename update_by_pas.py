...
def distal_pas_in_gb(gene_bed, pas_bed, gene_pas, tes_for_na=True, overwrite=False):
    ...
    with xopen(gene_bed) as r, xopen(gene_pas, 'rt') as w: # bed6 
        for l in r:
            ...
            pas_list = list(pas_ix.fetch(f'{s[0]}:{s[1]}-{s[2]}')) # candidates
...
def main():
    if len(sys.argv) < 4:
        print(
        """
        Usage: python update_pas.py <gene.bed.gz> <pas.bed.gz> <out.bed>
        """
        )
        gene_bed, pas_bed, gene_pas = sys.argv[1:4]
        distal_pas_in_gb(gene_bed, pas_bed, gene_pas)
